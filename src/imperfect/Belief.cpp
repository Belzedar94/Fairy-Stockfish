...
bool BeliefState::is_king_capturable(const Position& pos) const {
    ...
    // First check if the position has exactly one king for the opponent
    // (pos.square<KING> asserts count == 1, so we must check first)
    if (pos.count<KING>(them) != 1)
        return true; // Invalid position (missing or multiple kings)
